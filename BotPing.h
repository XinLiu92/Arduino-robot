// ---------------------------------------------------------------------------
// BotPing Library - v1.5 - 08/15/2012
//
// AUTHOR/LICENSE:
...
// available ping and ultrasonic libraries causing the problem. The BotPing
...
//
// CONSTRUCTOR:
//   BotPing sonar(trigger_pin, echo_pin [, max_cm_distance])
...
//   BotPing::timer_us(frequency, function) - Call function every frequency microseconds.
//   BotPing::timer_ms(frequency, function) - Call function every frequency milliseconds.
//   BotPing::timer_stop() - Stop the timer.
//
// HISTORY:
...
//   you're also using BotPing's ping_timer because both use Timer2 interrupts).
...
//   doesn't give correct results (at least with ping sensors). The BotPing
...
#define ECHO_TIMER_FREQ 50      // Frequency to check for a ping echo (every 50uS is about 0.83cm accuracy).
#define PING_MEDIAN_DELAY 29    // Millisecond delay between pings in the ping_median method.

// Conversion from uS to distance (round result to nearest cm or inch).
#define BotPingConvert(echoTime, conversionFactor) (max((echoTime + conversionFactor / 2) / conversionFactor, (echoTime ? 1 : 0)))


class BotPing {
	public:
		BotPing(uint8_t trigger_pin, uint8_t echo_pin, int max_cm_distance = MAX_SENSOR_DISTANCE);
		unsigned int ping();
